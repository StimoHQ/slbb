...
- **Формат и тип контента в БД не хранится.** `Text` не имеет `format`/`type`: на MVP источник один (`Source.GUTENBERG`), формат один (plain-text). Не «восстанавливать» `TextFormat` для полноты; если реально понадобится второй формат/тип — это отдельное решение с миграцией, а не свободная колонка.
- **`sourceMeta` — доказательство происхождения, а не свалка метаданных.** При импорте книги туда кладётся `TextLoadMeta`: `{ languageSource: "source-header" | "libretranslate", origin: <url или путь файла>, rawLanguage?: <строка Language: из шапки> }` — то, по чему позже понимают, почему книга признана английской. Заголовок/автор/дата там не дублируются: `title` уже в `Text`, остальное выводится из `origin`. Поле nullable, поэтому новый ключ в нём — не миграция, а только договорённость формата JSON: ломающие изменения обсуждаются явно.
...
- **Импорт идемпотентен по `@@unique([source, sourceObjId])`.** `sourceObjId` — внешний id источника (для Gutenberg — номер книги из URL `epub/79501/pg79501.txt`). Реализовано: `create` + перехват `P2002` → `ConflictException` (409): повторный импорт той же книги не создаёт вторую строку. Перезаписи (upsert) намеренно нет — обновить уже лежащий текст можно только отдельным осознанным решением.
...
1. **Загрузка книги** → `Text` + `TextContent` одной вложенной записью (`content: { create: { content } }`) в `TextService.create()`. Текст берёт `GutenbergTxtLoaderService` (`src/modules/gutenberg_loader/`): читает поток целиком, но язык решает на первых байтах и обрывает загрузку при несовпадении — в БД неанглийский текст не попадает. Лоадер к БД не прикасается, а контракт для любых источников один: `TextLoader`/`TextLoadResult` в `src/modules/text/interfaces/` (там же `SupportedLanguage`), маппинг сырого обозначения языка — `toSupportedLanguage()` в `src/modules/text/utils/`.
